...
export default {
	...
	handler: async ({ command, respond }) => {
		...
		if (!groupName) {
			await respond(
				`Couldn't find the group name in your command.\nUsage: \`${command.text.split(' ')[0]} promote [group name] @user1 @user2 ...\``,
			);
			return;
		}
		...
		if (
			group.members.some(
				(member) => member.role === 'admin' && users.includes(member.userId),
			)
		) {
			await respond(
				`These users are already admins: ${group.members.filter((member) => member.role === 'admin' && users.includes(member.userId)).map((x) => `<@${x.userId}>`)}`,
			);
			return;
		}
		...
		await respond(`Promoted ${users.length} user(s) to admin`);
	},
